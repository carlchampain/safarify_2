...
class MapContainer extends Component {
  ...
  vernacularName = async function() {
    ...
    this.signal = axios.CancelToken.source();
    for (let i = counter - 5; i < species.length; i++) {
  ...
  loopForFallback = () => {
    for(let j=this.state.counter-5; j<this.state.commonName.length; j++) {
  ...
  onMarkerClick = (marker, e) => {
    //const activeMarkerXY = marker.position;
      this.setState({
  ...
  flickerCall = async function(){
    const { species, counter, photoUrl } = this.state;
    const flickerState = {
      isLoaded: true,
      isLoading: false,
      counter: counter + 5,
    ...
    const photoUrlArray = [];
    for (let i = counter - 5; i < species.length; i++) {
      const flickrURL = `https://api.flickr.com/services/rest/?method=flickr.photos.search&text=${species[i]}&api_key=${flickerKey}&per_page=1&format=json&sort=relevance&nojsoncallback=?`;
      await axios.get(flickrURL, {
        cancelToken: this.signal.token
      }).then(resFlickr => {
        // console.log('species from flickrCall: ', species[i]);
        ...
        if (photoLength > 0) {
          ...
          if (i === species.length - 1) {
                  this.setState({ photoUrl: photoUrl.concat(photoUrlArray), loadingUpdate: false, ...flickerState }, () => {
                      moveUpSearch();
                  });
          }
        } else {
  ...
  fetchSightingsAfterPlace = (viewportLatMax, viewportLatMin, viewportLngMin, viewportLngMax, countryCode, formattedAdress, lat, lng) => {
    ...
    this.getSpeciesInRadius(allResults);
  }
  ...
  homeClickAppCSS = () => {
    ...
    if (this.state.commonName.length === 0 && this.state.loading === false) this.setState({value: ''});
    
    window.requestAnimationFrame(() => clickedCloseBtn());
    if(document.getElementsByClassName('x2')[0]) window.requestAnimationFrame(() => this.addClassCloud());
  }
  ...
  hasClickedOnLogIn = () => {
      this.setState({ isLoggingIn: true, isAbout: false, isSavedAnimals: false, isAccountSettings: false }, () => aboutClickCSS());
  }
  ...
  likeClick = (event) => {
    ...
    if (user) {
      ...
      fire.writeUserData(this.state.place, comName, latName, flickerUrl, viewportLngMin, viewportLngMax, viewportLatMin, viewportLatMax, speciesKeySaved, lat, lng, uid, cCode);
  ...
  delAccountAfterLogIn = () => {
    this.setState({ isAccountSettings: true, deleteError: false}, () => moveUpSearch());
  }

  togglePopup = () => {
    this.setState({ showPopup: !this.state.showPopup}, () => moveUpSearch());
  }
  ...
  render() {
    ...
    if (noResultsError) {
      ...
      {navBar}
        <Suspense fallback={<LoadingContainer />}>
          <ErrorHandler errorMessage={'No results for this search! Click on the animal icon to reset.'} />
        </Suspense>
      </div>
    );
    }
    if (errorHandlingPlaces) {
      return (
        ...
        {navBar}
          <Suspense fallback={<LoadingContainer />}>
            <ErrorHandler errorMessage={'For a successful search, select a place from the suggestions! Click on the animal icon to try again.'} />
          </Suspense>
        </div>
      );
    }
    ...
    if (isMapVisible) {
      ...
      <div>
        <NavBarMap 
          ...
          title={animalNameInTitle ? animalNameInTitle : clickedSpecie}
        />
